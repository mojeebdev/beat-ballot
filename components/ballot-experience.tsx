...
export function BallotExperience() {
  ...
  return <main>
    ...
    <header className="site-header"><a className="wordmark" href="#top" aria-label="Beat Ballot home">BEAT<br />BALLOT<span>.</span></a><div className="header-status"><span className="status-led" /><span>{loading ? "LOADING LIVE TAPE" : "ROUND 01 OPEN"}</span></div><nav aria-label="Primary navigation"><a href="#arena">ARENA</a><a href="#method">METHOD</a><a href="#catalogue">INDEX</a>{game?.viewer.signedIn ? <button className="nav-button" onClick={() => void signOut()}>SIGN OUT</button> : <Link href="/auth/sign-in?returnTo=/#arena">SIGN IN</Link>}</nav></header>
    <section className="hero" id="top"><div className="hero__meta"><span>NIGERIA / 2026</span><span>SEASON 01</span></div><p className="kicker">A FAN-LED CULTURAL EXPERIMENT</p><h1>HIT<span>/</span>FOR<span>/</span>HIT</h1><div className="hero__bottom"><p className="hero__statement">Two deep catalogues. No official fight. Just the songs people would go to war for.</p><a className="arrow-action" href="#arena">ENTER THE BALLOT <span>↓</span></a></div><div className="hero__scoreline"><div><span>01</span> OLAMIDE</div><span className="scoreline-vs">VS</span><div>DAVIDO <span>02</span></div></div></section>
